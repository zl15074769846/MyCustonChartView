...
/**
 * Created by zhuliang on 2019/6/20.
 */
...
public class ChartView extends View {
    ...
    //折线图中人次的颜色
    private int countTextColor = 0xff333333;
    //xy坐标轴宽度
    private int xyLineWidth = dpToPx(1);
    //xy坐标轴文字大小
    private int xyTextSize = spToPx(13);
    //xy坐标轴人次大小
    private int xyTextCountSize = spToPx(15);
    //x轴各个坐标点水平间距
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.ChartLineView, defStyleAttr, 0);
        int count = array.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = array.getIndex(i);
            if (attr == R.styleable.ChartLineView_xy_line_color) {
                xyLineColor = array.getColor(attr, xyLineColor);

            } else if (attr == R.styleable.ChartLineView_xy_line_width) {
            ...
            } else if (attr == R.styleable.ChartLineView_xy_text_color) {
                xyTextColor = array.getColor(attr, xyTextColor);

            } else if (attr == R.styleable.ChartLineView_xy_text_size) {
            ...
            } else if (attr == R.styleable.ChartLineView_line_color) {
                lineColor = array.getColor(attr, lineColor);

            } else if (attr == R.styleable.ChartLineView_interval) {
            ...
            } else if (attr == R.styleable.ChartLineView_bg_color) {
                bgColor = array.getColor(attr, bgColor);

            } else if (attr == R.styleable.ChartLineView_select_circle_color) {
                selectCircleColor = array.getColor(attr, selectCircleColor);

            } else if (attr == R.styleable.ChartLineView_select_reminder_color) {
                selectReminderColor = array.getColor(attr, selectReminderColor);

            } else if (attr == R.styleable.ChartLineView_isScroll) {
                isScroll = array.getBoolean(attr, isScroll);
            } else if (attr == R.styleable.ChartLineView_show_position) {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        if (changed) {
            ...
            //Y轴文本的最大宽度
            float textYWdith = 0;
//                    getTextBounds(mYData.get(getListItemMaxIndex(mYData)) + "", mXYTextPaint).width();
...
//            }
            ...
            if (null!=dateList && dateList.size()>0) {
                xValueRect = getTextBounds(dateList.get(getListItemMaxIndex(dateList)).getCreateDate(), mXYTextPaint);
                //X轴文本高度
                float textXHeight = xValueRect.height();
                for (int i = 0; i < dateList.size(); i++) {
                    Rect rect = getTextBounds(dateList.get(i) + "", mXYTextPaint);
...
//                mXInit = mXOri + xValueRect.width() / 2 + dpToPx(3);
                mXInit = 4*mXOri +dpToPx(3);
                minXInit = width - (width - mXOri) * 0.1f - interval * (dateList.size() - 1);
                maxXInit = mXInit;
            }
        }
...
//        RectF rectF = new RectF(0, 0, 0, height);
    ...
    private void drawBrokenPoint(Canvas canvas) {
        ...
        for (int i = 0; i < dateList.size(); i++) {
            ...
            float y = mYOri - mYOri * (1 - 0.1f) * Integer.parseInt(dateList.get(i).getContent()) / Float.parseFloat(bean.getMax() + "") - 72;
            //绘制选中点
            if (i == selectIndex - 1) {
                mSpinnerLinePaint.setStyle(Paint.Style.FILL);
                mSpinnerLinePaint.setColor(lineColor);
                canvas.drawCircle(x, y, dp8, mSpinnerLinePaint);
                mSpinnerLinePaint.setColor(Color.WHITE);
                canvas.drawCircle(x, y, dp6, mSpinnerLinePaint);
                mSpinnerLinePaint.setColor(lineColor);
                drawFloatTextBox(canvas, x, y - dp8, Integer.parseInt(dateList.get(i).getContent()));
            }
            //绘制普通节点
            mSpinnerLinePaint.setStyle(Paint.Style.FILL);
//            mSpinnerLinePaint.setColor(Color.WHITE);
    ...
    private void drawXY(Canvas canvas) {
        ...
        for (int i = 0; i < dateList.size(); i++) {
            ...
            if (x >= mXOri) {//只绘制从原点开始的区域
                //绘制X轴文本
                String text = dateList.get(i).getCreateDate();
                Rect rect = getTextBounds(text, mXYTextPaint);
                if (i == selectIndex - 1) {
                    //画柱形
                    Paint mGreenPaint = new Paint();
                    mGreenPaint.setColor(Color.parseColor("#f5f5f5"));

                    RectF rectZhuXing = new RectF(x - 100, 0, x + 100 + rect.width() / text.length() / 2, this.getHeight());
                    canvas.drawRect(rectZhuXing, mGreenPaint);

                    mXYTextPaint.setColor(lineColor);
                    mXYTextPaint.setTextSize(xyTextCountSize);
                    canvas.drawText(dateList.get(i).getContent(), 0, dateList.get(i).getContent().length(), x - rect.width() / text.length(), mYOri + xyLineWidth + dpToPx(2) - 9 * rect.height() / 5, mXYTextPaint);
                    mXYTextPaint.setTextSize(xyTextSize);
                    canvas.drawText(text, 0, text.length(), x - rect.width() / 2, mYOri + xyLineWidth + dpToPx(2), mXYTextPaint);
                } else {
                    mXYTextPaint.setColor(countTextColor);
                    mXYTextPaint.setTextSize(xyTextCountSize);
                    canvas.drawText(dateList.get(i).getContent(), 0, dateList.get(i).getContent().length(), x - rect.width() / text.length(), mYOri + xyLineWidth + dpToPx(2) - 9 * rect.height() / 5, mXYTextPaint);
                    mXYTextPaint.setTextSize(xyTextSize);
                    mXYTextPaint.setColor(xyTextColor);
...
//        }
        for (int i = 0; i < dateList.size(); i++) {
            ...
            if (eventX >= x - dp12 && eventX <= x + dp12 &&
                    eventY >= y - dp12 && eventY <= y + dp12 && selectIndex != i + 1) {//每个节点周围范围内都是可点击区域
                selectIndex = i + 1;
                setOnitemClickListener.updateTime(i);
                invalidate();
                return;
            }
            //X轴刻度
            ...
            if (eventX >= x - rect.width() / 2 - dp12 && eventX <= x + rect.width() + dp12 / 2 &&
                    eventY >= y - 3 * rect.height() / 2 - dp12 && eventY <= y + rect.height() + dp12 && selectIndex != i + 1) {
                selectIndex = i + 1;
                setOnitemClickListener.updateTime(i);
                invalidate();
                return;
    ...
    private int getSelectIndexFromShowType(int showPositionType) {
        ...
        switch (showPositionType) {
            ...
            case MIDDLE:
                if (dateList.size() <= visibleScale) {
                    mSelectMiddle = middleIndex(dateList.size());
                } else {
    ...
    public void setSmoothIndex(int position) {
        if(selectIndex!=position+1) {
            if (selectIndex <3) {
                if (position > 2) {
                    mXInit -= interval * (position - 2);
                }
                else
                {
                    mXInit=108;
                ...
                if (position > selectIndex - 2) {
                    mXInit -= interval * (position - selectIndex+1);
                } else {
                    if (position > 2) {
                        mXInit += interval * (selectIndex - position-1);
                    }
                    else
                    {
                        mXInit=108;
    ...
    private List<DoctorJieZhenBean.DataBean.DateListBean> dateList = new ArrayList<>();

    public void setValue(DoctorJieZhenBean.DataBean bean,SetOnitemClickListener setOnitemClickListener) {
        ...
        if (null!=dateList && dateList.size()>0) {
            ...
            float textXHeight = xValueRect.height();
            for (int i = 0; i < dateList.size(); i++) {
                Rect rect = getTextBounds(dateList.get(i).getCreateDate() + "", mXYTextPaint);
                if (rect.height() > textXHeight)
                    textXHeight = rect.height();
                if (rect.width() > xValueRect.width())
                    xValueRect = rect;
            ...
            mYOri = (int) (height - dp2 - textXHeight - dp3 - xyLineWidth);
            mXInit = mXOri + xValueRect.width() / 2;
            minXInit = width - (width - mXOri) * 0.1f - interval * (dateList.size() - 1);
            maxXInit = mXInit;
        }
        requestLayout();
    ...
    public int getListItemMaxIndex(List<DoctorJieZhenBean.DataBean.DateListBean> data) {
        if (data == null || data.size() < 1) {
            return NULL_INDEX;
        }
        int max = (data.get(0).getCreateDate() + "").length();
        for (int i = 0; i < data.size(); i++) {
            String s = data.get(i).getCreateDate() + "";
            if (s.length() > max) {
                return i;
            }
        }
    ...
    private Point getSpinnerPoint(int valueIndex) {
        ...
        float y = mYOri - mYOri * (1 - 0.1f) * Integer.parseInt(dateList.get(valueIndex).getContent()) / Float.parseFloat(bean.getMax() + "") - 72;
